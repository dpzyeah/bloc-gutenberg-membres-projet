 ...
 * Crée des dictionnaires pour accès rapide par ID
 */
...
*/
function membersFromTokens(tokens, idToTitle, idToFeaturedMedia, idToLink) {
	return (
		(tokens || [])
			.map((t) => {
				if (typeof t === "number") {
...
export default function Edit({ attributes, setAttributes }) {
	...
	const { loading, idToTitle, idToFeaturedMedia, idToLink, suggestions } = listeDesMembres();
	...
	*/
	useEffect(() => {
		...
		const next = members.map((m) => ({
			...
			link: idToLink[m.id] || m.link || '',
		}));
		
		if (JSON.stringify(next) !== JSON.stringify(members)) {
			setAttributes({ members: next });
		}
	}, [idToTitle, idToFeaturedMedia, idToLink]); // eslint-disable-line react-hooks/exhaustive-deps
			...
			<InspectorControls>
				<PanelBody title={__("Paramètres", "cerse")} initialOpen>
					<FormTokenField
						...
						onChange={(newTokens) =>
							setAttributes({
								members: membersFromTokens(newTokens, idToTitle, idToFeaturedMedia, idToLink),
							})
						}
